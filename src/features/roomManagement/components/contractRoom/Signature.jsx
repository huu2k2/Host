...
const Signature = ({
  ...
  const [imagePreview, setImagePreview] = useState(
    defaultValue ? `data:image/jpeg;base64,${defaultValue}` : ""
  );
  const inputFileRef = useRef(null);

  useEffect(() => {
    setValue(defaultValue ? `data:image/jpeg;base64,${defaultValue}` : "");
    setImagePreview(defaultValue ? `data:image/jpeg;base64,${defaultValue}` : "");
  ...
  return (
    <div className="flex justify-between items-center mb-5 mt-5">
      ...
      <div className="flex items-center justify-center w-80">
        ...
        >
          {count === 1 ? "Đã tải chữ kí" : "Tải chữ kí"}
        </button>
      </div>
      ...
      >
        <Modal.Header className="w-full text-center bg-white">
          Tải hình ảnh
        </Modal.Header>
        <Modal.Body className="w-full flex flex-col justify-center items-center py-4 border bg-white">
        ...
        <Modal.Footer className="w-full flex justify-end gap-2 py-5 bg-white">
          <button
            onClick={openModalSign}
            type="button"
            className="text-white bg-rose-600 border border-rose-600 focus:outline-none hover:bg-rose-700 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-4 py-2 me-2 mb-2 w-40"
          >
            Tạo chữ kí
          </button>
          <button
